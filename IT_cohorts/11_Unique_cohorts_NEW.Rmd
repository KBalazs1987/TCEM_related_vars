...
pbsapply(rownames(genotypes), function(x) {
  alleles = genotypes[x,]
  writeLines(alleles, paste0(ext_folder1, "11out/", cohort_name, "/alleles/", x, ".txt"))
})
...
ids = list.files(paste0(ext_folder1, "11out/", cohort_name, "/tcem_mutated_nonamers/"))
chowell_data = read_excel(paste0(ext_folder1, "objects/aao4572_Chowell_SM-Table-S9.xlsx"))
clinical = chowell_data[chowell_data$Reference == "Snyder et al. 2014",c("Sample", "OS_Months", "OS_Event", "Cancer Type", "Drug Class", "MutCnt")]
class(clinical$OS_Months) = "numeric"
class(clinical$OS_Event) = "numeric"
mutcnt = pbsapply(ids, function(x) {
  load(paste0(ext_folder1, "11out/", cohort_name, "/tcem_mutated_nonamers/", x))
  length(unique(nonamers$mut))
})
all(names(mutcnt) == clinical$Sample)
cor.test(mutcnt, as.numeric(clinical$MutCnt), method = "spearman")
plot(mutcnt, as.numeric(clinical$MutCnt))
View(cbind(mutcnt, as.numeric(clinical$MutCnt)))
clinical$TMB = unname(mutcnt)

colnames(clinical)[c(1,4,5)] = c("sampleid", "Cancer.Type", "Drug.Class")

pat_char = clinical[,c("sampleid","OS_Months", "OS_Event", "Drug.Class", "Cancer.Type", "TMB")]
save(pat_char, file = paste0(ext_folder1, "11out/", cohort_name, "/pat_char"))

load(paste0(ext_folder1, "11out/", cohort_name, "/pat_ratio"))
cohort = cbind(pat_char, pat_ratio[match(pat_char$sampleid, rownames(pat_ratio)),])
save(cohort, file = paste0(ext_folder1, "11out/", cohort_name, "/cohort"))
```
...
colnames(clinical)[c(1,21)] = c("sampleid","OS_Event")
...
```{r}
#Snyder
load(paste0(ext_folder1, "11out/snyder/cohort"))
snyder = cohort
...
cohort = rbind(cbind(cohort = "snyder", snyder), cbind(cohort = "vanallen", vanallen), cbind(cohort = "hugo", hugo), cbind(cohort = "riaz", riaz))
save(cohort, file = paste0(ext_folder1, "11out/cohorts_melanoma"))
...
```{r}
load(paste0(ext_folder1, "11out/cohorts_melanoma"))
...
res = expand.grid(cohort = c("all", unique(cohort$cohort)), tcemv = tcemvars, tcemv.cutpoint = seq(0.05,0.6,0.05), tcemv_filter = c("no", "yes"), stringsAsFactors = F)
res_ftest = res
res %<>%
  mutate(m34n = NA, TMBcor_rho = NA, TMBcor_p = NA, TCEMvarcountcor_rho = NA, TCEMvarcountcor_p = NA, m3coef = NA, m3p = NA, m3warning = NA,
...
for(i in 1:nrow(res)) {
  tryCatch({  
    ...
    cohort_f %<>% 
      ...
      dplyr::select(all_of(c("sampleid", "OS_Months", "OS_Event", "TMB", "countvar", "tcemvar")))
    ...
    res$TCEMvarcountcor_rho[i] = cor.test(cohort_f$countvar, cohort_f$tcemvar, method = "spearman")$estimate
    res$TCEMvarcountcor_p[i] = cor.test(cohort_f$countvar, cohort_f$tcemvar, method = "spearman")$p.value
...
save(res, file = paste0(ext_folder1,"11out/survival_cohorts_melanoma"))
write.table(res, file = paste0(ext_folder1,"11out/survival_cohorts_melanoma.csv"), quote = F, sep = ";", dec = ".", row.names = F)
res_filt = res %>% 
  dplyr::filter(TMBcor_p>.05, m3p<.05, m4p<.05, m4lown>10, m4highn>10) %>% 
  dplyr::select(c(1:5,10,11,13:16,18,19,21,22)) %>% 
  dplyr::arrange(tcemv)
res_filt[,c(6,7,10:15)] = apply(res_filt[,c(6,7,10:15)], 2, function(x) round(x,3)) 
write.table(res_filt, file = paste0(ext_folder1,"11out/survival_cohorts_melanoma_signif.csv"), quote = F, sep = ";", dec = ".", row.names = F)
```

#Results:

```{r}
load(paste0(ext_folder1,"11out/survival_cohorts_melanoma"))
res %>% 
...
cor.test(cohort_f$countvar, cohort_f$tcemvar, method = "spearman")
